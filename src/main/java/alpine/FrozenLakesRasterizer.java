...
@RegisterPlugin
public class FrozenLakesRasterizer implements WorldRasterizerPlugin {
    private Block water;
    private Block ice;
    ...
    public void generateChunk(CoreChunk chunk, Region chunkRegion) {
        ...
        for (Vector3i position: chunkRegion.getRegion()) {
            ...
            if (position.y() < seaLevel - 1 && position.y() > surfaceHeight) {
                chunk.setBlock(ChunkMath.calcBlockPos(position), water);
            } else if (position.y() < seaLevel && position.y() > surfaceHeight) {
